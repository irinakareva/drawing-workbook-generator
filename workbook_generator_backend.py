...
class WorkbookBuilder:
    ...
    def _advance_pages(self, count=1):
        self.page_cursor += count

    def _duplex_enabled(self):
        return self.print_layout == 'duplex_facing'

    def _ensure_even_page_start(self, doc, upcoming_title='next exercise'):
        if self._duplex_enabled() and self.page_cursor % 2 == 1:
            self.parity_filler_page(doc, upcoming_title)

    def parity_filler_page(self, doc, upcoming_title='next exercise'):
        self.page_header(doc, 'WORKBOOK NOTES', 'Warm-up / notes page', subtitle='Inserted automatically so the next full-page reference/drawing pair can face correctly in duplex printing.', compact=True)
        self.task_box(doc, f'This page is here to keep duplex layout friendly. You can use it for warm-up marks, notes, thumbnail planning, or extra practice before {upcoming_title.lower()}.')
        for _ in range(10):
            p = doc.add_paragraph()
            p.paragraph_format.space_after = Pt(0)
            r = p.add_run('________________________________________________________________________________________')
            r.font.name = 'Aptos'
            r.font.size = Pt(9)
            r.font.color.rgb = RGBColor.from_string('C2C7CC')
        self.page_break(doc)
        self._advance_pages(1)
    ...
    def large_pair(self, doc,image_path,key,subject,title,task,kind='clear',level=None,upside=False):
        self._ensure_even_page_start(doc, title)
        ratio=self.image_ratio(image_path)
    ...
    def progressive_focus(self, doc,image_path,key,subject):
        ...
        if self._duplex_enabled():
            self._ensure_even_page_start(doc, 'Progressive focus')
            for st in range(1,5):
                self.page_header(doc,subject,f'Progressive focus - {stage_names[st]}',subtitle=f'Stage {st} of 4 - duplex spread', compact=True)
                ...
                draw_subtitle = f'Stage {st} of 4 - facing working page'
                self.page_header(doc,subject,'Progressive focus - working drawing',draw_subtitle, compact=True)
                if st == 1:
                    draw_task = 'Begin your drawing here using the broadest shapes only. In duplex mode each stage gets a facing drawing page so no reference prints on the back of a working page.'
                else:
                    draw_task = 'Continue developing the drawing using the reference on the facing page. A fresh facing drawing page is repeated in duplex mode so the spread stays usable when printed double-sided.'
                self.task_box(doc,draw_task)
                self.ptext(doc,'WORKING DRAWING',7.8,True,MUTED,2,WD_ALIGN_PARAGRAPH.CENTER)
                self.add_picture_center(doc,self.blank_asset(image_path,key,'coarse'),w,h)
                self.page_break(doc)
            self._advance_pages(8)
            return
...
def convert_docx_to_pdf(docx_path, output_pdf=None):
    ...
    cmd=['libreoffice','--headless','--convert-to','pdf','--outdir',str(outdir),str(docx_path)]
