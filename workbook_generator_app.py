...
print_layout = st.selectbox(
    'Print layout',
    options=[
        ('duplex_facing', 'Duplex - facing pages (recommended)'),
        ('single_sided', 'Single-sided / no duplex adjustments'),
    ],
    format_func=lambda x: x[1],
    index=0,
    help='In duplex mode, full-page reference/drawing pairs are shifted so the reference is on the left page and the drawing page faces it on the right. Progressive focus also becomes duplex-safe: each stage gets its own facing working page so no reference prints on the back of a drawing page.'
)[0]
...
if uploaded_files and st.button('Generate workbook', type='primary'):
    ...
    builder = WorkbookBuilder(asset_dir, print_layout=print_layout)
