...
PALE = (0.95, 0.96, 0.97)
LINE = (0.72, 0.74, 0.77)
BACKEND_VERSION = "duplex-v2"


class WorkbookBuilder:
    """Generate black-and-white drawing-practice workbooks directly as PDF."""

    def __init__(self, asset_dir, print_layout="duplex_facing"):
        self.asset_dir = Path(asset_dir)
        self.asset_dir.mkdir(parents=True, exist_ok=True)
        self.cache = {}
        self.print_layout = print_layout
        # Tracks the page currently being drawn. In a normal duplex book,
        # even-numbered pages are left pages and odd-numbered pages are right pages.
        self.page_number = 1
        # Image paths listed here have already been framed/cropped by the user.
        # They must not go through the automatic content-cropping pass again.
        self.preserve_crop_paths = set()
    ...
    def load_crop(self, image_path):
        ...
        preserve_crop = image_path in self.preserve_crop_paths
        ck = ("crop", image_path, preserve_crop)
        if ck in self.cache:
    ...
    def _duplex_enabled(self):
        return self.print_layout == "duplex_facing"

    def _practice_filler_page(self, c, upcoming_title="next large exercise"):
        """Use an otherwise necessary parity page as low-pressure drawing space."""
        y = self._header(
            c,
            "EXTRA PRACTICE",
            "Warm-up / extra drawing",
            f"This page keeps the next reference and drawing on facing pages for duplex printing. Use it if useful; otherwise skip it before {upcoming_title}.",
        )
        y = self._task_box(c, "Loose marks, thumbnails, proportion checks, or a quick repeat are all good uses for this page.", y)

        # One large open drawing box.
        box_top = y - 8
        box_h = max(300, box_top - 135)
        c.setStrokeColorRGB(*LINE)
        c.setLineWidth(0.6)
        c.rect(MARGIN, box_top - box_h, CONTENT_W, box_h, fill=0, stroke=1)

        # Three small thumbnail boxes at the bottom.
        gap = 10
        thumb_w = (CONTENT_W - 2 * gap) / 3
        thumb_h = 74
        thumb_y = FOOTER_Y + 22
        for i in range(3):
            x = MARGIN + i * (thumb_w + gap)
            c.rect(x, thumb_y, thumb_w, thumb_h, fill=0, stroke=1)
        self._finish_page(c)

    def _ensure_left_reference_page(self, c, upcoming_title="next large exercise"):
        """For duplex long-edge printing, references should start on an even (left) page."""
        if self._duplex_enabled() and self.page_number % 2 == 1:
            self._practice_filler_page(c, upcoming_title)
    ...
    def large_pair(self, c, image_path, key, subject, title, task, kind="clear", level=None, upside=False):
        # A duplex spread is page EVEN on the left + page ODD on the right.
        # Ensure the reference begins on the even page so the drawing faces it.
        self._ensure_left_reference_page(c, title)
        ratio = self.image_ratio(image_path)
        paths = [
    ...
    def progressive_focus(self, c, image_path, key, subject):
        ...
        if self._duplex_enabled():
            # Keep the pedagogical idea of ONE evolving drawing, while also making
            # duplex printing physically usable: all four reference stages share
            # the left page, and one large working drawing faces them on the right.
            self._ensure_left_reference_page(c, "Progressive focus")
            y = self._header(
                c, subject, "Progressive focus - four reference stages",
                "Work through these in order on the single drawing page facing this spread: very blurred -> medium blur -> nearly sharp -> sharp.",
            )
            y = self._task_box(
                c,
                "Do not restart. Start with the largest masses, then add only what each sharper stage genuinely reveals. The same coarse scaffold appears throughout.",
                y,
            )
            gap = 10
            cell_w = (CONTENT_W - gap) / 2
            cell_h = (y - FOOTER_Y - 26 - gap) / 2
            top_y = y - 6
            for idx, st in enumerate(range(1, 5)):
                row, col = divmod(idx, 2)
                x = MARGIN + col * (cell_w + gap)
                cell_top = top_y - row * (cell_h + gap)
                cell_bottom = cell_top - cell_h
                c.setStrokeColorRGB(*LINE)
                c.setLineWidth(0.5)
                c.rect(x, cell_bottom, cell_w, cell_h, fill=0, stroke=1)
                ...
                c.drawString(x + 6, cell_top - 12, f"{st}. {stage_names[st].upper()}")
                c.setFont("Helvetica", 6.2)
                task_lines = self._wrap(stage_tasks[st], "Helvetica", 6.2, cell_w - 12)
                ty = cell_top - 22
                for line in task_lines[:2]:
                    c.drawString(x + 6, ty, line)
                    ty -= 7
                iw, ih = self._fit_box(ratio, cell_w - 12, cell_h - 46)
                ix = x + (cell_w - iw) / 2
                iy = cell_bottom + 6
                self._draw_image(c, self.asset(image_path, key, f"focus{st}", "coarse"), ix, iy, iw, ih)
            self._finish_page(c)

            y = self._header(
                c, subject, "Progressive focus - one evolving drawing",
                "Keep this same drawing page open while you move through all four reference stages on the facing page.",
            )
            y = self._task_box(
                c,
                "Stage 1: biggest masses. Stage 2: major divisions and overlaps. Stage 3: principal contours. Stage 4: selected detail. Do not restart between stages.",
                y,
            )
        ...
        # Single-sided mode keeps the original sequence: one working sheet followed
        # by four full-size reference pages.
    ...
    def subject_core(self, c, spec):
        ...
        if spec.get("preserve_crop", False):
            self.preserve_crop_paths.add(str(image_path))
        key = spec["key"]
